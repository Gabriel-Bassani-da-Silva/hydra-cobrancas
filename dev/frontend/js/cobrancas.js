...
/* ── Botão Tornar Pedra ─────────────────────────────────────── */
function btnPedra(item, tipo) {
    if (tipo !== 'clientes') return '';
    const id = item.ID_CONTATO_BLING;
    return `<button type="button" class="btn-pedra" onclick="togglePedraJS(${id})" title="Tornar Pedra" style="background:none; border:none; color:#ef4444; cursor:pointer; padding:5px; margin-left: 8px;">
    ...
    Object.values(data).forEach(item => {
        ...
        html += `
            <tr class="expandable-row" data-id="${id}">
                ...
                <td class="cr-col-acoes" style="display: flex; align-items: center; justify-content: center;">
                    ${btnCobrar(item, tipo, nome)}
                    ${btnPedra(item, tipo)}
                </td>
            </tr>
        `;
...
window.togglePedraJS = function(id) {
    if(!confirm("Deseja marcar este cliente como 'Pedra'? Ele não aparecerá mais nas cobranças.")) return;
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
    fetch(`${BASE}/contatos/toggle-pedra`, {
        method: 'POST',
        headers: { 
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': csrfToken,
    ...
    .then(data => {
        if (data.ok) {
            alert('Cliente marcado como pedra com sucesso!');
            location.reload();
        } else {
            alert('Erro ao marcar como pedra.');
        }
    })
    .catch(() => alert('Erro de comunicação com o servidor.'));
};
