    ...
    data.forEach(item => {
        ...
        colsHtml += `
            ...
            <td class="cr-col-acoes" style="text-align:right;">
                ${(aba === 'clientes' || aba === 'pedras') ? btnPedra(item) : ''}
...
function btnPedra(item) {
    const id = item.ID_CONTATO_BLING;
    const isPedra = item.PEDRAS == 1;
    const color = isPedra ? '#94a3b8' : '#ef4444';
    const title = isPedra ? 'Remover Pedra' : 'Tornar Pedra';
    return `<button type="button" class="btn-pedra" onclick="togglePedraJS(${id}, ${isPedra ? 1 : 0})" title="${title}" style="background:none; border:none; color:${color}; cursor:pointer; padding:5px; margin-right: 4px; vertical-align:middle;">
...
window.togglePedraJS = function(id, isPedra) {
    const msg = isPedra 
        ? "Deseja remover este cliente da aba 'Pedras'?" 
        : "Deseja marcar este cliente como 'Pedra'? Ele só aparecerá na aba Pedras.";
    if(!confirm(msg)) return;
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
    fetch(`${BASE}/contas-receber/toggle-pedra`, {
        method: 'POST',
        headers: { 
    ...
    .then(data => {
        if (data.ok) {
            ...
            alert('Erro ao marcar/desmarcar pedra.');
        }
    })
...
// As funções de baixa manual foram movidas para baixa_manual.js
