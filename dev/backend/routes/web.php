...
Route::get('/migrate-pedido', function() {
    try {
        ...
        // Copia o ID do Bling para a nova coluna (colunas ORIGEM/ID_BLING já existem)
        $pdo->exec("UPDATE PEDIDO SET ID_BLING = ID_PEDIDO WHERE ID_BLING IS NULL;");

        // Drop da FK que bloqueia o MODIFY
        $pdo->exec("ALTER TABLE VINCULO_COBRANCA_PEDIDO DROP FOREIGN KEY FK_VINCULO_COBR_PED_PEDIDO;");

        // Transforma ID_PEDIDO em AUTO_INCREMENT
        ...
        // Índice único para ON DUPLICATE KEY UPDATE funcionar na sincronização
